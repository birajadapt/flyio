...
print("---- birajlog OUTSPEED_HTTP_PORT", os.getenv("OUTSPEED_HTTP_PORT"), flush=True)
print(">>>>>>>>>>> birajlog files in current directory", os.listdir("."), flush=True)
...
class VoiceBot:
    ...
    async def run(
        self, audio_input_stream: sp.AudioStream, text_input_stream: sp.TextStream
    ):
        """
        ...
        text_input_stream = sp.map(
            text_input_stream, lambda x: json.loads(x).get("content")
        )
        ...
        token_aggregator_stream: sp.TextStream = self.token_aggregator_node.run(
            llm_token_stream
        )
        tts_stream: sp.AudioStream = self.tts_node.run(token_aggregator_stream)

        chat_history_stream = sp.filter(
            chat_history_stream, lambda x: json.loads(x).get("role") != "user"
        )
